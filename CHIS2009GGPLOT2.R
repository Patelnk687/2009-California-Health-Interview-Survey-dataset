#CHIS Data 2009
# the data file was too large and i could not add it to github but it can be downloaded
# through <http://healthpolicy.ucla.edu/chis/data/public-use-data-file/Pages/2009.aspx#download>
...
library("ggplot2")
library("sas7bdat")
adult<-read.sas7bdat("adult.sas7bdat", debug=TRUE)
##Exploring Data
...
str(adult)
# Age histogram
ggplot(adult, aes(x=SRAGE_P)) + geom_histogram(binwidth=1)
ggplot(adult, aes(x=BMI_P)) +  geom_histogram(binwidth=1)
...
# Melt results$residuals and store as
...
p1 <- p %+% DF_all + geom_text(aes(x = max(xmax),  y = yposn,
                                   label = FILL),size = 3, hjust = 1, show.legend  = FALSE)
p1
